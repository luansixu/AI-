...
export class Game {
  ...
  constructor() {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.code === 'KeyI') this.tryCraftLeather(); // I键合成皮衣
  ...
  private tryCraftLeather() {
    if (this.player.craftLeatherCoat()) {
        ...
        feedbackManager.showMeme("“羊皮紧贴着皮肤，严寒似乎退却了一些。”", 4000);
    } else {
  ...
  private tryInteract() {
    ...
    if (nearestItem) {
      ...
      if (type === 'Berry') {
        ...
        feedbackManager.triggerMeme('eat');
      } else if (type === 'Wood') {
        ...
        soundManager.playPickup();
        vfxManager.createBurst(this.scene, nearestItem.mesh.position, 0xeeeeee, 10);
      }
    ...
    for (let i = this.animals.length - 1; i >= 0; i--) {
        ...
        if (attackPos.distanceTo(animal.pos) < attackRange) {
            ...
            if (animal.takeHit(40, this.player.mesh.position)) {
                ...
                if (animal.type === 'Sheep') this.spawnItem('Fur', animal.pos.clone().add(new THREE.Vector3(0.5, 0, 0.5)));
  ...
  private animate = () => {
    ...
    if (this.scene.fog) {
        if (pos.x > 30 && pos.z < -30) {
            ...
            // 只有没有熔火核心才扣血
            if (!this.player.state.inventory.moltenCore) {
